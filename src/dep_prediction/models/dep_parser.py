...
class DependencyModel(Model):
    ...
    def forward(self,  # type: ignore
                ...
                dep_label: torch.LongTensor = None,
        ...
        logits = self.feedforward_layer(selected_indices)
        ...
        if dep_label is not None:
            self.metrics['accuracy'](logits, dep_label)

            loss = self.loss(logits, dep_label)
            output["loss"] = loss

        # if dep_head is not None:
        #     self.metrics['accuracy'](logits, torch.argmax(dep_head, dim=2), mask.float())
        #
        #     loss = self.loss(logits, dep_head)
        #     output["loss"] = loss
