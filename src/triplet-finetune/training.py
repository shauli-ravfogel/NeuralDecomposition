...
def train(model, cca_model, training_generator, dev_generator, loss_fn, cca_loss_fn, optimizer, scheduler, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        for (sents, ids) in t:
            ...
            loss, diff, batch_good, batch_bad, norm = loss_fn(vecs, sents, ids)
...
def evaluate(model, cca_model, loss_fn, cca_loss_fn, dev_generator):
    ...
    for i, (sents, ids) in enumerate(t):

        with torch.no_grad():

            vecs = model(sents)

            loss, diff, batch_good, batch_bad, norm = loss_fn(vecs, sents, ids, i, evaluation = True)
