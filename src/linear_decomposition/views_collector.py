...
class CollectorBase(object):
    ...
    def __init__(self, path, view_size, method, exclude_function_words):
        """
                Parameters
                -------------------------
                path: str, required.
                      The path to the dataset that contain equivalent sentences.
                      The file is HDF5 format. Each group (listed by a string index) is one set of equivalent sentences,
                       and contains datasets "vecs" (ELMO representations of the words), "sents" and "content indices"
                        (indices of content words)
                     (The format is described above in 'Equivalent_sentences_group'")
                """
    ...
    def collect_views(self, output_file: str):
        pbar = tqdm.tqdm(total=self.view_size, ascii=True)
        views = []
        i = 0
